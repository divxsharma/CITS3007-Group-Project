...
bool account_lookup_by_userid(const char *userid, account_t *result) {
    if (strcmp(userid, "test1") == 0) {
        ...
        account_update_password(result, "Str0ng!Pass1");
        return true;
    }
    ...
    if (strcmp(userid, "test3") == 0) {
        ...
        result->expiration_time = time(NULL) + 24*60*60;
        // Use the real password hashing function
        account_update_password(result, "Str0ng!Pass3");
        return true;
    }
    ...
    if (strcmp(userid, "test4") == 0) {
      ...
      // Use the real password hashing function
      account_update_password(result, "Str0ng!Pass4");
      return true;
    }
    ...
    if (strcmp(userid, "test5") == 0) {
      ...
      result->expiration_time = time(NULL) + 24*60*60; // Set an expired account
      // Use the real password hashing function
      account_update_password(result, "Str0ng!Pass5");
      return true;
    }
    ...
    if (strcmp(userid, "test6") == 0) {
      ...
      result->expiration_time = time(NULL) + 24*60*60; // Set an expired account
      // Use the real password hashing function
      account_update_password(result, "Str0ng!Pass6");
      return true;
    }
    ...
    if (strcmp(userid, "test7") == 0) {
      ...
      result->expiration_time = time(NULL) + 24*60*60; // Set an expired account
      // Use the real password hashing function
      account_update_password(result, "Str0ng!Pass7");
      return true;
    }
    ...
    if (strcmp(userid, "test9") == 0) {
      ...
      result->expiration_time = time(NULL) + 24*60*60; // Set an expired account
      // Use the real password hashing function
      account_update_password(result, "Str0ng!Pass9");
      return true;
    }
    return false;
}
...
START_TEST(test_handle_login_success) {
    ...
    login_result_t result = handle_login("test1", "Str0ng!Pass1", client_ip, login_time, client_output_fd, log_fd, &session);
...
START_TEST(test_handle_login_user_not_found) {
  ...
  login_result_t result = handle_login("test2", "Str0ng!Pass1", client_ip, login_time, client_output_fd, log_fd, &session);
  log_message(LOG_INFO, "[TEST] Login result: %d", result);
...
START_TEST(test_handle_login_user_banned) {
  ...
  login_result_t result = handle_login("test3", "Str0ng!Pass1", client_ip, login_time, client_output_fd, log_fd, &session);
  log_message(LOG_INFO, "[TEST] Login result: %d", result);
...
START_TEST(test_handle_login_user_expired) {
  ...
  login_result_t result = handle_login("test4", "Str0ng!Pass1", client_ip, login_time, client_output_fd, log_fd, &session);
  log_message(LOG_INFO, "[TEST] Login result: %d", result);
...
START_TEST(test_handle_login_user_too_many_failed_logins) {
  ...
  login_result_t result = handle_login("test5", "Str0ng!Pass5", client_ip, login_time, client_output_fd, log_fd, &session);
  log_message(LOG_INFO, "[TEST] Login result: %d", result);
...
START_TEST(test_handle_login_null_username_correct_password) {
  ...
  ip4_addr_t client_ip = {127001}; // Localhost IP
  log_message(LOG_INFO, "[TEST] Testing correct username incorrect password user login: test7");
  login_result_t result = handle_login(NULL, "Str0ng!Pass6", client_ip, login_time, client_output_fd, log_fd, &session);
  log_message(LOG_INFO, "[TEST] Login result: %d", result);
  ck_assert_int_eq(result, LOGIN_FAIL_USER_NOT_FOUND);
} END_TEST
...
START_TEST(test_handle_login_correct_username_correct_password_exactly_10_prior_failed_logins) {
  ...
  login_result_t result = handle_login("test9", "Str0ng!Pass9", client_ip, login_time, client_output_fd, log_fd, &session);
  log_message(LOG_INFO, "[TEST] Login result: %d", result);
  ck_assert_int_eq(result, LOGIN_SUCCESS);
} END_TEST
